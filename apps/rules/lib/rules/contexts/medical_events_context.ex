defmodule Rules.Contexts.MedicalEventsContext do
  ...
  given_(~r/^care level (?<is_equal>(=|!=)) care level from request$/, fn
    ...
    # care level should not be equal to MSP care level request
    state, %{is_equal: "!="} ->
      {:ok, state}
  end)

  @doc "Request access to resource by action name"
  when_(~r/^I require (?<action_name>(read|write|update|action)) access$/, fn
    state, %{action_name: _action_name} ->
      {:ok, state}
  end)

  @doc "Verify access to resource by action name"
  then_(~r/^I (?<has_access>(can|can't)) (?<action_name>(read|write|update|do action))$/, fn
